			...
			for (Vertex v : this.lstVertices) {
				...
				for (Vertex u : this.lstVertices) {
					...
					//	if (v1_OnBorder & u_OnBorder)continue;
						...
						double rforce = 1;
						if (deltaLength == EPSILON ||deltaLength < 30) {rforce =(repulsionF(Math.abs(deltaLength))* 100);}
						else{
						 rforce = repulsionF(Math.abs(deltaLength)); }    // repulsion force (distance)
		...
		for (int i = 0; i < this.size; i++) {
			for (int j = 0; j < this.size; j++){
				...
				if (i!=j){
					...
					double myD = Math.abs((tmp[i].distance(tmp[j]))) ;
					double fixD = (double)20;
...
private double getCons(double K){
	return (37038.432128607375/k) ;
}
